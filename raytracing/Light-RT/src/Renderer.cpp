...
glm::vec3 Renderer::getColor(const Ray& ray, const std::vector<std::shared_ptr<Primitive>>& scene,int depth) {
	...
	glm::vec3 emitted = record.material->emitted(record.uv, record.pos);

	// return emitted if not scatter
	if (!record.material->scatter(ray, record, scatteredRay, attenuation)) {
		return emitted;
	}

	// return emitted plus ray color
	return emitted + attenuation * getColor(scatteredRay, scene, depth - 1);
}
