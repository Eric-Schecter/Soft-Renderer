...
layout(binding = 1) uniform directionalLight{
    DirectionalLight uDirectionalLight[DIRECTIONAL_LIGHT_COUNT];
};
layout(binding = 2) uniform ambientLight{
    AmbientLight uAmbientLight[AMBIENT_LIGHT_COUNT];
};
layout(binding = 3) uniform pointLight{
    PointLight uPointLight[POINT_LIGHT_COUNT];
};
layout(binding = 4) uniform spotLight{
    SpotLight uSpotLight[SPOT_LIGHT_COUNT];
};
layout(binding = 5) uniform cameraPos{
    vec3 uCameraPos;
};
layout(binding = 6) uniform material{
    Material uMaterial;
};
layout(binding = 7) uniform sampler2D diffuseMap;
