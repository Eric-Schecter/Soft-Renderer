...
void Renderer::createDescriptorSetLayout() {
	...
	vk::DescriptorSetLayoutBinding cameraPosLayoutBinding = {
		5,
		vk::DescriptorType::eUniformBuffer,
		1,
		vk::ShaderStageFlagBits::eFragment,
...
void Renderer::createUniformBuffers() {
	...
	uniformBuffersDirectionalLight.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMemoryDirectionalLight.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMappedDirectionalLight.resize(MAX_FRAMES_IN_FLIGHT);

	uniformBuffersAmbientLight.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMemoryAmbientLight.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMappedAmbientLight.resize(MAX_FRAMES_IN_FLIGHT);

	uniformBuffersSpotLight.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMemorySpotLight.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMappedSpotLight.resize(MAX_FRAMES_IN_FLIGHT);

	uniformBuffersPointLight.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMemoryPointLight.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMappedPointLight.resize(MAX_FRAMES_IN_FLIGHT);

	uniformBuffersCameraPos.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMemoryCameraPos.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMappedCameraPos.resize(MAX_FRAMES_IN_FLIGHT);

	uniformBuffersMaterial.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMemoryMaterial.resize(MAX_FRAMES_IN_FLIGHT);
	uniformBuffersMappedMaterial.resize(MAX_FRAMES_IN_FLIGHT);
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
		...
		createBuffer(
			...
			vk::BufferUsageFlagBits::eUniformBuffer,
			vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
			uniformBuffersDirectionalLight[i],
			uniformBuffersMemoryDirectionalLight[i]
		);
		uniformBuffersMappedDirectionalLight[i] = device->mapMemory(uniformBuffersMemoryDirectionalLight[i], 0, bufferSize);

		bufferSize = sizeof(AmbientLight);
		createBuffer(
			bufferSize,
			vk::BufferUsageFlagBits::eUniformBuffer,
			vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
			uniformBuffersAmbientLight[i],
			uniformBuffersMemoryAmbientLight[i]
		);
		uniformBuffersMappedAmbientLight[i] = device->mapMemory(uniformBuffersMemoryAmbientLight[i], 0, bufferSize);

		bufferSize = sizeof(SpotLight);
		createBuffer(
			bufferSize,
			vk::BufferUsageFlagBits::eUniformBuffer,
			vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
			uniformBuffersSpotLight[i],
			uniformBuffersMemorySpotLight[i]
		);
		uniformBuffersMappedSpotLight[i] = device->mapMemory(uniformBuffersMemorySpotLight[i], 0, bufferSize);

		bufferSize = sizeof(PointLight);
		createBuffer(
			bufferSize,
			vk::BufferUsageFlagBits::eUniformBuffer,
			vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
			uniformBuffersPointLight[i],
			uniformBuffersMemoryPointLight[i]
		);
		uniformBuffersMappedPointLight[i] = device->mapMemory(uniformBuffersMemoryPointLight[i], 0, bufferSize);

		bufferSize = sizeof(camera->pos);
		createBuffer(
			bufferSize,
			vk::BufferUsageFlagBits::eUniformBuffer,
			vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
			uniformBuffersCameraPos[i],
			uniformBuffersMemoryCameraPos[i]
		);
		uniformBuffersMappedCameraPos[i] = device->mapMemory(uniformBuffersMemoryCameraPos[i], 0, bufferSize);
		...
		createBuffer(
			...
			vk::BufferUsageFlagBits::eUniformBuffer,
			vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
			uniformBuffersMaterial[i],
			uniformBuffersMemoryMaterial[i]
		);
		uniformBuffersMappedMaterial[i] = device->mapMemory(uniformBuffersMemoryMaterial[i], 0, bufferSize);
	}
}
...
void Renderer::createDescriptorSets() {
	...
	for (size_t i = 0; i < swapChainImages.size(); ++i) {
		...
		vk::DescriptorBufferInfo directionalLightInfo = {
			uniformBuffersDirectionalLight[i],
			0,								// offset
			sizeof(DirectionalLight)		// range
		};

		vk::DescriptorBufferInfo ambientLightInfo = {
			uniformBuffersAmbientLight[i],
			0,							// offset
			sizeof(AmbientLight)		// range
		};

		vk::DescriptorBufferInfo pointLightInfo = {
			uniformBuffersPointLight[i],
			0,						// offset
			sizeof(PointLight)		// range
		};

		vk::DescriptorBufferInfo spotLightInfo = {
			uniformBuffersSpotLight[i],
			0,						// offset
			sizeof(SpotLight)		// range
		};

		vk::DescriptorBufferInfo cameraPosInfo = {
			uniformBuffersCameraPos[i],
			0,						// offset
			sizeof(glm::vec3)		// range
		};

		vk::DescriptorBufferInfo materialInfo = {
			uniformBuffersMaterial[i],
			0,						// offset
			sizeof(Material)		// range
		};
		...
		memcpy(uniformBuffersMappedDirectionalLight[i], &lights.directionalLights[0], sizeof(lights.directionalLights[0]));
		memcpy(uniformBuffersMappedAmbientLight[i], &lights.ambientLights[0], sizeof(lights.ambientLights[0]));
		memcpy(uniformBuffersMappedPointLight[i], &lights.pointLights[0], sizeof(lights.pointLights[0]));
		memcpy(uniformBuffersMappedSpotLight[i], &lights.spotLights[0], sizeof(lights.spotLights[0]));
		memcpy(uniformBuffersMappedCameraPos[i], &camera->pos, sizeof(camera->pos));
		memcpy(uniformBuffersMappedMaterial[i], &material, sizeof(material));
	}
}
