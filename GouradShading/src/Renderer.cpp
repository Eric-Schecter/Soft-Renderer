...
void Renderer::render(const std::vector<Vertex>& vertices,const std::vector<uint32_t>& indices,const Camera* camera, const glm::mat4& modelMatrix) {
	...
	for (auto i = 0; i < indices.size(); i+=3) {
		...
		glm::vec4 p1cs = shader->vertex(vertices[indices[i]].position,vertices[indices[i]].normal,0);
		glm::vec4 p2cs = shader->vertex(vertices[indices[static_cast<size_t>(i) + 1]].position, vertices[indices[static_cast<size_t>(i) + 1]].normal,1);
		glm::vec4 p3cs = shader->vertex(vertices[indices[static_cast<size_t>(i) + 2]].position, vertices[indices[static_cast<size_t>(i) + 2]].normal,2);
